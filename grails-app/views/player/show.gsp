...
-->
				...
				<li class="fieldcontain">
					<span id="scores-label" class="property-label"><g:message code="player.scores.label" default="Recent Scores" /></span>
									
						<g:each in="${playerInstance.scores.sort{a,b-> (a.dateCreated>b.dateCreated ? -1 : 1)}}" var="s" status="i">
							<g:if test="${ i<10 }">
								<span class="property-value" aria-labelledby="scores-label"><g:link controller="score" action="show" id="${s.id}">${s?.toStringWithGame().encodeAsHTML()}</g:link></span>
							</g:if>
						</g:each>
