			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="gameName" title="${message(code: 'game.gameName.label', default: 'Game Name')}" />
						
						<!-- <g:sortableColumn property="scores" title="${message(code: 'game.scores.label', default: 'High Score')}" /> -->
						<th>High Score</th> <!-- non sortable column -->
				...
				<g:each in="${gameInstanceList}" status="i" var="gameInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${gameInstance.id}">${fieldValue(bean: gameInstance, field: "gameName")}</g:link></td>
						<g:if test="${gameInstance?.scores }">
							<td><g:link controller="score" action="show" id="${gameInstance.highScore().id}">${gameInstance.getHighScore()}</g:link></td>
						</g:if>
						<g:else>
							<td>${gameInstance.getHighScore()}</td>
						</g:else>
