...
<div class="fieldcontain ${hasErrors(bean: scoreInstance, field: 'player', 'error')} required">
	...
	<g:select id="player" name="player.id" from="${rcadeserver.Player.list()}" optionKey="id" required="" value="${scoreInstance?.player?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: scoreInstance, field: 'game', 'error')} required">
	...
	<g:select id="game" name="game.id" from="${rcadeserver.Game.list()}" optionKey="id" required="" value="${scoreInstance?.game?.id}" class="many-to-one"/>
</div>

<div class="fieldcontain ${hasErrors(bean: scoreInstance, field: 'cabinetID', 'error')} ">
	<label for="cabinetID">
		<g:message code="score.cabinetID.label" default="Cabinet ID" />
		
	</label>
	<g:textField name="cabinetID" value="${scoreInstance?.cabinetID}"/>
</div>

<div class="fieldcontain ${hasErrors(bean: scoreInstance, field: 'arcadeName', 'error')} ">
	<label for="arcadeName">
		<g:message code="score.arcadeName.label" default="Arcade Name" />
