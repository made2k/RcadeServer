			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="cabinetID" title="${message(code: 'score.cabinetID.label', default: 'Cabinet ID')}" />
					
						<g:sortableColumn property="dateCreated" title="${message(code: 'score.dateCreated.label', default: 'Date Created')}" />
					
						<g:sortableColumn property="arcadeName" title="${message(code: 'score.arcadeName.label', default: 'Arcade Name')}" />
				...
				<g:each in="${scoreInstanceList}" status="i" var="scoreInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${scoreInstance.id}">${fieldValue(bean: scoreInstance, field: "player")}</g:link></td>
					
						<td>${fieldValue(bean: scoreInstance, field: "score")}</td>
					
						<td>${fieldValue(bean: scoreInstance, field: "game")}</td>
					
						<td>${fieldValue(bean: scoreInstance, field: "cabinetID")}</td>
					
						<td><g:formatDate date="${scoreInstance.dateCreated}" /></td>
					
						<td>${fieldValue(bean: scoreInstance, field: "arcadeName")}</td>
